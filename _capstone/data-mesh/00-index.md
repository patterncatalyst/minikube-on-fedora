---
title: "Capstone: a data mesh on minikube"
order: 0
description: "The map — what the capstone builds, the reading order, and a link to each page in the set. Start here."
---

<!-- DRAFT SKELETON (Phase C) — the SET LANDING / index for the capstone. -->
<!-- INDEX/landing — short scenario-setting prose (reuse old 17-capstone.md intro, lines 8-23) + the data-mesh-overview diagram + the described list of the nine docs below. Pull the one-paragraph 'what you end up with' from old 'What the capstone builds' (old lines 1176-1198). Point at bootstrap-capstone.sh (CAP-035) for one-command bring-up and §1 for host prereqs. -->

![Capstone: a data mesh on minikube]({{ '/assets/diagrams/17-capstone-data-mesh.svg' | relative_url }})

This is the §17 capstone, organized as its own set of pages — read in order the
first time, or jump to the concept you came for.

## The set

<!-- DRAFT: short described list linking the nine docs below, in the
     Statelessness-Compendium style. Links use the capstone collection permalinks. -->

1. [Concepts &amp; principles]({{ '/capstone/data-mesh/01-concepts/' | relative_url }})
2. [Kubernetes as the substrate]({{ '/capstone/data-mesh/02-kubernetes-substrate/' | relative_url }})
3. [Services &amp; data products]({{ '/capstone/data-mesh/03-services-and-data-products/' | relative_url }})
4. [Contracts &amp; the catalog]({{ '/capstone/data-mesh/04-contracts-and-catalog/' | relative_url }})
5. [The data planes]({{ '/capstone/data-mesh/05-data-planes/' | relative_url }})
6. [Progressive delivery &amp; mTLS]({{ '/capstone/data-mesh/06-progressive-delivery-mtls/' | relative_url }})
7. [Elastic &amp; resilient]({{ '/capstone/data-mesh/07-elastic-and-resilient/' | relative_url }})
8. [Observability]({{ '/capstone/data-mesh/08-observability/' | relative_url }})
9. [Anti-patterns]({{ '/capstone/data-mesh/09-anti-patterns/' | relative_url }})

